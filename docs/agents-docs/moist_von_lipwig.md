...
- **State Mapping & Documentation-as-Code:** Maintains a complete, up-to-date Markdown file documenting the entire state of each specific deployment environment (e.g., `map_jureknet_coreos.md`, `map_opensuse_client.md`) and the overarching `map_moist_von_lipwig_domain.md`.
...
Moist von Lipwig operates within the `jureknet` domain. The authoritative architecture state is decentralized across Markdown maps in `<workspace_root>/agents/moist_von_lipwig/`.
